...
const Wrapper = styled.div`
  overflow-x: hidden; /* Hide overflowing tags */
...
const ArrowButton = styled.button`
  ...
  background: radial-gradient(circle at left, white 40%, transparent 90%);
`;

export const Suggestions = () => {
  const [arrowVisible, setArrowVisible] = useState({ left: false, right: true });
  ...
  return (
    <Container>
      {arrowVisible.left && <ArrowButton left onClick={scrollLeft}>{'<'}</ArrowButton>}
      ...
      {arrowVisible.right && <ArrowButton right onClick={scrollRight}>{'>'}</ArrowButton>}
    </Container>
  );
};
